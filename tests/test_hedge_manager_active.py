...
from daytrade_bot.datetime import datetime, timezone, timedelta
from daytrade_bot.hedge_manager import manage_active_hedge
...
def test_manage_active_hedge_close_by_drawdown(monkeypatch):
    ...
    monkeypatch.setattr("hedge_manager.close_position", mock_close_position)

    config = {"hedge_close_drawdown_cash": 10.0, "hedge_cooldown_minutes": 600}
    result = manage_active_hedge(state, trade, 2450.0, now, config, logger)
    assert called["closed"]
    assert not result["hedge_manager_active"]
